...
function EmojiSelector({ onEmojiClick, selectedEmoji }: { selectedEmoji: string | null; onEmojiClick: (clickedIcon: string) => void }) {
    ...
    return (
        <View style={{ backgroundColor: "#282A2C", borderRadius: 10, paddingHorizontal: 5, paddingBottom: 10 }}>
            <ScrollView
                horizontal
                showsHorizontalScrollIndicator={false}
                style={[{ paddingTop: 5, marginBottom: 15, borderBottomWidth: 1, borderColor: colors.line }]}>
                {keys.map((key) => {
            ...
            <Animated.FlatList
                ...
                style={{ height: 200 }}
                numColumns={parseInt(`${(width - 20) / EMOJI_SIZE}`)}
                renderItem={({ item }) => {
                    return (
                        <Pressable
                            style={[
